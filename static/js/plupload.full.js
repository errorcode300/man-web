		...
		g.extend(this, {
			...
			init : function() {
				...
				B.bind("FilesAdded", function(D, G) {
					//start扩展修改--只能单选文件  --zhongqh 
					if(D.settings.one_file && D.settings.one_file === true && D.files.length>=1){
					parent.alert("只能选择一个文件");
					return;
					}
					//end
					//weijx修改 文件扩展错误提示合法后缀名
					var F, E, I = 0, J, Z,H = s.filters;
					if (H && H.length) {
						J = [];
						Z=[];
						g.each(H, function(K) {
							...
							s
									.bind(
											...
											function(A) {
												...
												if (J) {
													var C = A.settings.browse_button_hover, E = A.settings.browse_button_active, B = A.features.triggerDialog ? J
															: K;
													if (C) {
														l.addEvent(B,
																"mouseover",

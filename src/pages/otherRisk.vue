...
<template>
  <div id="otherRisk">
    ...
    <el-scrollbar ref="scrollbar">
      <div class="o-chartBox">
        <el-col v-for="item in cols" :key="item.id">
          <p class="title">{{item.title}}</p>
          <div v-if="item.id == 1" :id="'o-myChart' + item.id" style="height: calc((100vh - 285px) * 0.4);"></div>
          <div v-else :id="'o-myChart' + item.id" style="height: calc((100vh - 195px) * 0.4);"></div>
          ...
          <el-table-column
            ...
            <template v-slot='scope'>
              <el-popover placement="right" width="300" trigger="click">
                ...
                <span slot="reference">
                  {{scope.row.idxNm}}
                  <span v-if="scope.row.unit && (scope.row.unit != '%')">({{scope.row.unit}})</span>
                </span>
              </el-popover>
            </template>
          </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      cols: [ // 排版
        ...
        {
          id: 2,
          title: '全部同业资产与一级资本净额比',
          idxStr: 'GK111004'
        },
        {
          id: 3,
          title: '增速差（房地产贷款-各项贷款）',
          idxStr: 'GS121008'
        }
      ],
  ...
  watch: {
    ...
    '$store.state.isCollapse' () {
      if (this.$store.state.currentPage.name === 'otherRisk') {
        setTimeout(() => {
  ...
  methods: {
    // 初始化页面
    initPage: function () {
      this.getIdxData(this.cols[0].idxStr, 'o-myChart1') // 请求指标数据
      this.getIdxDateData(this.cols[1].idxStr, 'o-myChart2') // 请求过去十二个月历史数据
      this.getIdxDateData(this.cols[2].idxStr, 'o-myChart3') // 请求过去十二个月历史数据
      this.getTableData('otherRisk') // 获取表格数据
    },
    // 时点修改
    changeDate: function () {
      this.initPage()
    ...
    callbackChart: function (idxNos, moduleId, data) {
      let dataObj
      if (moduleId === 'o-myChart1') {
        dataObj = Object.values(data)
      } else if (moduleId === 'o-myChart2') {
        dataObj = dataFunc.dateToMonth(Object.values(data))
      } else if (moduleId === 'o-myChart3') {
        dataObj = dataFunc.dateToMonth(Object.values(data))
      }
    ...
    initEcharts: function (idxNos, moduleId, dataObj) {
      ...
      switch (moduleId) {
        case 'o-myChart1': {
          // chart1
          let chart1 = this.$echarts.init(document.getElementById('o-myChart1'))
          let extend = dataFunc.getGaugeRange(dataObj[0].currVal, 0, 30, 20)
          let nums = {
            'min': extend.min,
            'max': extend.max,
          ...
          let option1 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '房地产贷款占比_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
            ...
            series: [
              {
                ...
                detail: {
                  ...
                  offsetCenter: [0, '45%'],
                  formatter: function (value) {
                    return isNaN(value) ? '{a|\n房地产贷款占比}' : '{b|' + value.toFixed(2) + dataObj[0].unit + '}' + '{a|\n房地产贷款占比}'
                  },
                  color: '#949494',
                  fontSize: 14,
                  rich: {
              ...
              },
              {
                ...
                axisLine: {
                  ...
                  lineStyle: {
                    color: [
                      [nums.value / nums.max, new this.$echarts.graphic.LinearGradient(0, 0, 1, 0, [{
                        offset: 0,
                        color: '#F3F1FE'
                      },
                      {
        ...
        case 'o-myChart2': {
          // chart2
          let chart2 = this.$echarts.init(document.getElementById('o-myChart2'))
          ...
          let option2 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '全部同业资产与一级资本净额比_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
        ...
        case 'o-myChart3': {
          // chart3
          let chart3 = this.$echarts.init(document.getElementById('o-myChart3'))
          let extend = dataFunc.getLineAxis(dataObj[0].values)
          let chart3numbers = {
            'xdate': dataObj[0].dateMonth,
            'ydata': dataObj[0].values,
          ...
          let option3 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '增速差(房地产贷款-各项贷款)_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
    ...
    callbackTable: function (moduleId, dataObj) {
      if (moduleId === 'otherRisk') {
        // 排序
        // dataObj = dataObj.sort(function (a, b) {
        //   return a.secNm >= b.secNm ? 1 : -1
        // })
        // 调用统一方法生成表格数组
        let tableDataArr = dataFunc.toTableArr(dataObj)
        this.tableData = tableDataArr
        this.getSpanArr(this.tableData, 'program', 0) // 合并单元格
      }
    },
    // 单元格合并
    getSpanArr (data, attribute, columnIndex) {
      let spanArr = []
      ...
      this.spanArr = this.spanArr.concat(spanArr)
    },
    objectSpanMethod: function ({ row, column, rowIndex, columnIndex }) {
      let mergeNum
      this.spanArr.map(i => {
        if (i.rowIndex === rowIndex && i.columnIndex === columnIndex) {
          mergeNum = {
            rowspan: i.rowspan,
            colspan: i.colspan
          }
    ...
    discountHeaderStyle: function ({ row, column, rowIndex, columnIndex }) {
      if (rowIndex === 1) {
        return { display: 'none' }
      }
    },
    ...
    tableExportToExcel: function () {
      let that = this
      // 获取表格二维数组
      let arr = [['项目', '指标名称', '监管标准', '辖内平均', '本期数值', '频度', '比上期', '比年初', '比同期']]
      this.tableData.map(i => {
        ...
        currentArr.push(i.program, i.idxNm, i.reguStd, i.juriAvg, i.currVal, freq, i.combqVal + i.unit, i.combyVal + i.unit, i.comlyVal + i.unit)
        arr.push(currentArr)
      })
      lib.exportData({
        content: arr
        ...
        .then(res => {
          if (res) {
            ...
            const fileName = '其他风险_' + that.selectedOrgStr + '_' + that.selectedDate + '.xls' // 文件名称
    ...
    tableExportToPdf: function () {
      ...
      lib.exportDataPdf({
        ...
        .then(res => {
          const blob = new Blob([res]) // 创建blob对象
          const fileName = '其他风险_' + that.selectedOrgStr + '_' + that.selectedDate + '.pdf' // 文件名称
          if ('download' in document.createElement('a')) {
            const link = document.createElement('a')
            link.download = fileName
            link.style.display = 'none'
  ...
  */
  mounted () {
    ...
    window.onresize = function () {
      if (that.$store.state.currentPage.name === 'otherRisk') {

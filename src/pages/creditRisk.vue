    ...
    <el-scrollbar ref="scrollbar">
      <div class="c-chartBox">
        <el-col v-for="item in cols" :key="item.id">
          ...
          <div v-if="item.id == 1" :id="'c-myChart' + item.id" style="height: calc((100vh - 285px) * 0.4);"></div>
          <div v-else :id="'c-myChart' + item.id" style="height: calc((100vh - 195px) * 0.4);"></div>
...
export default {
  ...
  watch: {
    '$store.state.isCollapse' () {
      if (this.$store.state.currentPage.name === 'creditRisk') {
        setTimeout(() => {
  ...
  methods: {
    // 初始化页面
    initPage: function () {
      this.getIdxData(this.cols[0].idxStr, 'c-myChart1') // 请求指标数据
      this.getIdxDateData(this.cols[1].idxStr, 'c-myChart2') // 请求过去十二个月历史数据
      this.getIdxDateData(this.cols[2].idxStr, 'c-myChart3') // 请求过去十二个月历史数据
      this.getTableData('creditRisk')
    },
    // 时点修改
    changeDate: function () {
      this.initPage()
    ...
    callbackChart: function (idxNos, moduleId, data) {
      let dataObj
      if (moduleId === 'c-myChart1') {
        dataObj = Object.values(data)
      } else if (moduleId === 'c-myChart2') {
        dataObj = dataFunc.dateToMonth(Object.values(data))
      } else if (moduleId === 'c-myChart3') {
        dataObj = dataFunc.dateToMonth(Object.values(data))
      }
    ...
    initEcharts: function (idxNos, moduleId, dataObj) {
      ...
      switch (moduleId) {
        case 'c-myChart1': {
          let chart1 = this.$echarts.init(document.getElementById('c-myChart1'))
          let extend = dataFunc.getGaugeRange(dataObj[0].currVal, 0, 15, 5, 15)
          let nums = {
            'min': extend.min,
            'max': extend.max,
          ...
          let option1 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '不良贷款率_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
            ...
            series: [
              ...
              },
              {
                ...
                axisLabel: {
                  ...
                  distance: 12,
                  formatter: function (value) {
                    if (Math.abs(value - nums.standard) < nums.splitNumber / 30 * nums.interVal) {
                      return ''
                    }
                    return value
                  },
        ...
        case 'c-myChart2': {
          // chart2
          let chart2 = this.$echarts.init(document.getElementById('c-myChart2'))
          ...
          let option2 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '逾期90天以上贷款与不良贷款比例_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
        ...
        case 'c-myChart3': {
          // chart3
          let chart3 = this.$echarts.init(document.getElementById('c-myChart3'))
          let extend = dataFunc.getLineAxis(dataObj[0].values)
          let chart3numbers = {
            'xdate': dataObj[0].dateMonth,
            'ydata': dataObj[0].values,
          ...
          let option3 = {
            ...
            toolbox: {
              feature: {
                saveAsImage: {
                  name: '正常贷款迁徒率(调整后)_' + that.selectedOrgStr + '_' + that.selectedDate,
                  title: '保存'
                }
              },
              right: '0.5%'
    ...
    tableExportToExcel: function () {
      let that = this
      // 获取表格二维数组
      let arr = [['指标名称', '监管标准', '辖内平均', '本期数值', '口径', '频度', '比上期', '比年初', '比同期']]
      this.tableData.map(i => {
        let currentArr = []
        let dataRgType = i.dataRgType
        if (dataRgType === '01') {
          dataRgType = '法人'
        } else if (dataRgType === '02') {
          dataRgType = '并表'
        }
        let freq = i.freq
        if (freq === '0') {
        ...
        currentArr.push(i.idxNm, i.reguStd, i.juriAvg, i.currVal, dataRgType, freq, i.combqVal + i.unit, i.combyVal + i.unit, i.comlyVal + i.unit)
        arr.push(currentArr)
      })
      lib.exportData({
        content: arr
        ...
        .then(res => {
          if (res) {
            ...
            const fileName = '信用风险_' + that.selectedOrgStr + '_' + that.selectedDate + '.xls' // 文件名称
    ...
    tableExportToPdf: function () {
      ...
      lib.exportDataPdf({
        ...
        .then(res => {
          const blob = new Blob([res]) // 创建blob对象
          const fileName = '信用风险_' + that.selectedOrgStr + '_' + that.selectedDate + '.pdf' // 文件名称
          if ('download' in document.createElement('a')) {
            const link = document.createElement('a')
            link.download = fileName
            link.style.display = 'none'
  ...
  */
  mounted () {
    ...
    window.onresize = function () {
      if (that.$store.state.currentPage.name === 'creditRisk') {
